            ...
            <div class="faqly-card">
                <div class="faqly-pro-plugin">
                    <div class="faqly-po-img-wrap">
                        <img src="<?php echo esc_url(FAQLY_PLUGIN_URL . 'assets/images/faqly-pro-plugin.png'); ?>"
                            alt="Upgrade To Pro!" />
                    </div>
...
<script type="text/javascript">
    document.addEventListener('DOMContentLoaded', function () {
        ...
        async function displaybundleData() {
            ...
            const bundleResponse = await fetchDataBySearchQuery(
                "WordPress Theme Bundle | 60+ Premium"
            );

            if (bundleResponse?.data?.products?.length) {
                const bundleProduct = bundleResponse.data.products.find(
                    p => p.node.handle === 'wordpress-theme-bundle'
                );

                if (bundleProduct) {
                    renderProduct(bundleProduct.node);
                }
            }

            const lifetimeResponse = await fetchDataBySearchQuery("lifetime access");

            if (lifetimeResponse?.data?.products?.length) {
                const lifetimeProduct = lifetimeResponse.data.products.find(
                    p => p.node.handle === 'wordpress-theme-package'
                );

                if (lifetimeProduct) {
                    renderProduct(lifetimeProduct.node);
                }
            }
        }
