...
// Nonce verification helper for admin page checks
if (!function_exists('faqly_is_templates_page')) {
    function faqly_is_templates_page()
    {
        if (isset($_GET['page'])) {
            if (is_admin() && function_exists('check_admin_referer')) {
                if (isset($_GET['_wpnonce']) && !wp_verify_nonce(sanitize_text_field(wp_unslash($_GET['_wpnonce'])), 'faqly_admin_nonce')) {
                    return false;
                }
            }
        }
        return isset($_GET['page']) && $_GET['page'] === 'templates_page';
    }
}

if (!function_exists('faqly_is_special_admin_page')) {
    function faqly_is_special_admin_page($hook)
    {
        if (is_admin()) {
            if (function_exists('check_admin_referer') && isset($_GET['_wpnonce']) && !wp_verify_nonce(sanitize_text_field(wp_unslash($_GET['_wpnonce'])), 'faqly_admin_nonce')) {
        ...
        if (isset($_GET['page'])) {
            ...
            return in_array($page, ['templates_page', 'faqly_tools', 'faqly_woocommerce', 'faqly-license-key'], true);
        }
...
function faqly_custom_popup_html()
{
    ...
    <div id="faqly-popup-overlay">
        <div id="faqly-popup-content">
            ...
            <h2>
                <?php echo esc_html('Elevate Your Website with Premium Themes from $45'); ?>
            </h2>
            <div class="faqly-popup-wrap">
                <a href="<?php echo esc_url(admin_url('edit.php?post_type=faqly_faq_group&page=templates_page')); ?>"
                    class="button button-primary faqly-popup-template-btn">
                ...
                <a href="<?php echo esc_url(FAQLY_PLUGIN_MAIN_URL) . 'products/wordpress-theme-bundle'; ?>" target="_blank"
                    class="button button-primary faqly-popup-bundle-btn">
                    <?php echo esc_html('Get Theme Bundle'); ?>
                </a>
            </div>
        </div>
    </div>
...
add_action('admin_enqueue_scripts', function () {
    ...
    wp_register_style('faqly-admin-styles', false, array(), FAQLY_PLUGIN_VERSION);
...
function faqly_enqueue_faq_admin_scripts($hook)
{
    ...
    if (faqly_is_special_admin_page($hook)) {
...
function faqly_enqueue_block_editor_assets()
{
    ...
    wp_localize_script(
        ...
        array(
            ...
            'nonce' => wp_create_nonce('faqly_templates_nonce')
        )
    );
    ...
    <div class="notice is-dismissible faqly-upsell-banner">
        ...
        <div class="faqly-notice-banner-wrap faqly-upsell-banner-container">
            <div class="faqly-notice-left-img faqly-upsell-banner-content">
                <h1>
                    <?php echo esc_html('WORDPRESS THEMES BUNDLE'); ?>
                </h1>
                <p>
                    <?php echo esc_html('GET OVER 60+ RESPONSIVE WORDPRESS THEMES FOR ONLY $79!'); ?>
                </p>
            </div>

            <div class="faqly-notice-btn faqly-upsell-banner-btn">
                <a class="faqly-buy-btn" target="_blank"
                    href="<?php echo esc_url(FAQLY_PLUGIN_MAIN_URL . 'products/wordpress-theme-bundle'); ?>">
