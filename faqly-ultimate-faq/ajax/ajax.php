...
function faqly_get_templates() {
    // Verify nonce
    if (!isset($_POST['nonce']) || !wp_verify_nonce(sanitize_text_field(wp_unslash($_POST['nonce'])), 'faqly_templates_nonce')) {
    ...
    $cursor = isset($_POST['cursor']) && is_string($_POST['cursor']) ? sanitize_text_field(wp_unslash($_POST['cursor'])) : null;
...
function faqly_load_faq_content() {
    try {
        // Verify nonce
        if (!isset($_POST['nonce']) || !wp_verify_nonce(sanitize_text_field(wp_unslash($_POST['nonce'])), 'faqly_ajax_nonce')) {
...
function faqly_export_data() {
    try {
        if (!isset($_POST['nonce']) || !wp_verify_nonce(sanitize_text_field(wp_unslash($_POST['nonce'])), 'faqly_export_nonce')) {
            wp_send_json_error('Security check failed');
            return;
        }
        ...
        switch ($export_type) {
            case 'faqs':
                $post_type = 'faqly_faq';
                $filename = 'faqly_faqs_export_' . gmdate('Y-m-d_H-i-s') . '.json';
                break;
            case 'groups':
                $post_type = 'faqly_faq_group';
                $filename = 'faqly_groups_export_' . gmdate('Y-m-d_H-i-s') . '.json';
                break;
            default:
...
function faqly_import_data() {
    try {
        // Verify nonce
        if (!isset($_POST['nonce']) || !wp_verify_nonce(sanitize_text_field(wp_unslash($_POST['nonce'])), 'faqly_import_nonce')) {
            wp_send_json_error('Security check failed');
            return;
        }
        ...
        $import_data = sanitize_text_field(wp_unslash($_POST['data']));
